...
| LLM | Anthropic Messages API (`WebClient` 직접 호출), 없어도 동작 |
...
키를 넣으면 라우팅과 슬롯 추출에 LLM 이 쓰인다. 키가 없으면 각각
키워드 매칭과 질문으로 자동 대체된다 — 서버는 어느 쪽이든 뜬다.
...
GET    /api/v1/models                   등록된 원자 모델 목록
```
...
```bash
./gradlew test              # 75개, Docker/API 키 불필요
...
| `template` | 템플릿-SES 정합성 위반 케이스 |
| `scenario` | 계층 PES 평탄화, EIC/EOC 전개 |
| `llm` | WireMock — 코드펜스 제거, 파싱 재시도, 재시도 대상 상태코드 |
...
**11. Spring Data 리포지토리를 파일마다 하나씩 둔다.**
...
| 6 | 확장 — 서브태스크 DAG, 시각화 프론트엔드, 템플릿 저작 도구 | 미착수 (선택) |
